...
class ShortcutRecorderNSView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let color: NSColor
        if isRecording {
            label = NSLocalizedString("shortcut.recorder.recording", comment: "")
            color = NSColor.secondaryLabelColor
        } else if !hasShortcut {
            label = NSLocalizedString("shortcut.recorder.click_to_set", comment: "")
    ...
    override func accessibilityLabel() -> String? {
        return NSLocalizedString("accessibility.shortcut_recorder", comment: "")
    }

    override func accessibilityValue() -> Any? {
        if isRecording {
            return NSLocalizedString("shortcut.recorder.recording", comment: "")
        }
        let hasShortcut = currentKeyCode != UInt16.max || !currentModifiers.isEmpty
        if !hasShortcut {
            return NSLocalizedString("shortcut.recorder.click_to_set", comment: "")
        }
        return ShortcutRecorderNSView.displayString(
            keyCode: currentKeyCode,
            modifiers: currentModifiers,
            showDoubleTap: showDoubleTap
        )
