...
struct PhrasedSettings: Codable, Equatable {
    static var settingsWereReset = false
    ...
    var isSystemAudio: Bool { audioSource == "systemAudio" }
    ...
    mutating func migrate() {
        ...
        // Migrate historyMaxEntries → historyRetentionDays (count-based → date-based)
        if historyMaxEntries != nil { historyMaxEntries = nil }
    ...
    static func load(from url: URL = PhrasedSettings.defaultStorageURL()) throws -> PhrasedSettings {
        ...
        var s = try JSONDecoder().decode(PhrasedSettings.self, from: patchedData)
        s.migrate()
        let needsSave = s.hasLegacyFields
        let migratedPlaintextAPIKeys = s.migrateProfileAPIKeysToKeychain()
        s.restoreProfileAPIKeysFromKeychain()
        if needsSave || migratedPlaintextAPIKeys { try? s.save(to: url) }
        return s
    }
    ...
    private static func patchMissingFields(in data: Data) throws -> Data {
        ...
        let defaultData = try JSONEncoder().encode(PhrasedSettings())
        guard let defaults = try JSONSerialization.jsonObject(with: defaultData) as? [String: Any] else {
            return data
        }
        if json["launchAtLogin"] == nil { json["launchAtLogin"] = defaults["launchAtLogin"] }
        if json["playCompletionSound"] == nil { json["playCompletionSound"] = defaults["playCompletionSound"] }
        if json["appLanguage"] == nil { json["appLanguage"] = defaults["appLanguage"] }
        // Migrate: default output mode changed from "copy" to "inject"
        if json["defaultOutputMode"] as? String == OutputMode.copy.rawValue {
            json["defaultOutputMode"] = OutputMode.inject.rawValue
        }
    ...
    private var hasLegacyFields: Bool {
        ...
        localProfiles.contains(where: { !$0.apiKey.isEmpty }) ||
        asrProviderID != nil ||
        asrProfiles.contains(where: { !$0.apiKey.isEmpty }) ||
    ...
    private mutating func clearProfileAPIKeys() {
        for index in localProfiles.indices {
            localProfiles[index].apiKey = ""
        }
        for index in asrProfiles.indices {
            asrProfiles[index].apiKey = ""
        }
    }

    private mutating func restoreProfileAPIKeysFromKeychain() {
        for index in localProfiles.indices {
            localProfiles[index].loadKeyFromKeychain()
        }
        for index in asrProfiles.indices {
            asrProfiles[index].loadKeyFromKeychain()
        }
    }

    private mutating func migrateProfileAPIKeysToKeychain() -> Bool {
        var migrated = false
        for profile in localProfiles where !profile.apiKey.isEmpty {
            profile.saveKeyToKeychain()
            migrated = true
        }
        for profile in asrProfiles where !profile.apiKey.isEmpty {
            profile.saveKeyToKeychain()
            migrated = true
        }
        return migrated
