# Phrased v2 Implementation Plan
...
**Architecture:** Full rewrite into `Sources/Phrased/` with 5 focused layers (App / Input / Core / Confirm / Output). Reuse `AudioCapture.swift` and `SpeechTranscriber.swift` as-is. Delete all old interview-specific code.
...
**Keep (copy as-is):**
- `Sources/InterviewCopilot/AudioCapture.swift` → `Sources/Phrased/Input/AudioCapture.swift`
- `Sources/InterviewCopilot/SpeechTranscriber.swift` → `Sources/Phrased/Input/SpeechTranscriber.swift`

**New files:**
- `Sources/Phrased/App/main.swift`
...
- `Sources/Phrased/Core/OllamaClient.swift`
- `Sources/Phrased/Core/UserProfile.swift`
...
- `Sources/Phrased/Output/ClipboardOutput.swift`

**Delete (old interview code, after new code builds):**
- `Sources/InterviewCopilot/` entire directory

**Update:**
- `Package.swift` — change path from `Sources/InterviewCopilot` to `Sources/Phrased`, add test target
...
- Create: `Sources/Phrased/` directory structure

- [ ] **Step 1: Create new directory structure**

```bash
cd /Users/helm/Projects/InterviewCopilot
mkdir -p Sources/Phrased/App
mkdir -p Sources/Phrased/Input
mkdir -p Sources/Phrased/Core
mkdir -p Sources/Phrased/Confirm
mkdir -p Sources/Phrased/Output
mkdir -p Tests/PhrasedTests
```

- [ ] **Step 2: Copy reusable files**

```bash
cp Sources/InterviewCopilot/AudioCapture.swift Sources/Phrased/Input/
cp Sources/InterviewCopilot/SpeechTranscriber.swift Sources/Phrased/Input/
```
...
Create `Sources/Phrased/App/main.swift`:
...
Create `Sources/Phrased/App/AppDelegate.swift`:
...
```bash
git add Package.swift Sources/Phrased/
git commit -m "chore: restructure project to Sources/Phrased layout"
```
...
**Files:**
- Create: `Sources/Phrased/Core/OllamaClient.swift`
- Create: `Tests/PhrasedTests/OllamaClientTests.swift`
...
Expected: FAIL — `Phrased` module has no `OllamaClient`
...
```bash
git add Sources/Phrased/Core/OllamaClient.swift Tests/PhrasedTests/OllamaClientTests.swift
...
**Files:**
- Create: `Sources/Phrased/Core/UserProfile.swift`
- Create: `Tests/PhrasedTests/UserProfileTests.swift`
...
```bash
git add Sources/Phrased/Core/UserProfile.swift Tests/PhrasedTests/UserProfileTests.swift
...
**Files:**
- Create: `Sources/Phrased/Core/IntentProcessor.swift`
- Create: `Tests/PhrasedTests/IntentProcessorTests.swift`
...
```bash
git add Sources/Phrased/Core/IntentProcessor.swift Tests/PhrasedTests/IntentProcessorTests.swift
...
**Files:**
- Create: `Sources/Phrased/Output/ClipboardOutput.swift`
- Create: `Tests/PhrasedTests/ClipboardOutputTests.swift`
...
```bash
git add Sources/Phrased/Output/ClipboardOutput.swift Tests/PhrasedTests/ClipboardOutputTests.swift
...
**Files:**
- Create: `Sources/Phrased/Confirm/ConfirmViewModel.swift`
...
```bash
git add Sources/Phrased/Confirm/ConfirmViewModel.swift
...
**Files:**
- Create: `Sources/Phrased/Confirm/ConfirmWindow.swift`
...
**Files:**
- Create: `Sources/Phrased/Input/InputWindow.swift`
...
**Files:**
- Create: `Sources/Phrased/Input/HotkeyManager.swift`
...
class HotkeyManager {
    ...
    private func register() {
        ...
        // Local monitor (when Phrased is focused)
...
```bash
git add Sources/Phrased/Input/HotkeyManager.swift
...
**Files:**
- Create: `Sources/Phrased/App/StatusBarController.swift`
...
class StatusBarController {
    ...
    init(onOpen: @escaping () -> Void) {
        ...
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "quote.bubble", accessibilityDescription: "Phrased")
...
Replace `Sources/Phrased/App/AppDelegate.swift`:
...
```bash
git add Sources/Phrased/App/StatusBarController.swift Sources/Phrased/App/AppDelegate.swift
...
**Files:**
- Modify: `Phrased.entitlements`
- Create: `Sources/Phrased/Resources/Info.plist`
- Modify: `Package.swift` — add resources

- [ ] **Step 1: Update entitlements for mic + speech**

Replace `Phrased.entitlements`:
...
<dict>
    ...
    <string>Phrased uses speech recognition to transcribe your voice input.</string>
    <key>NSMicrophoneUsageDescription</key>
    <string>Phrased uses your microphone for voice input.</string>
</dict>
</plist>
```
...
.build/release/Phrased &
```

Expected: Phrased icon appears in menu bar.
...
```bash
git add Phrased.entitlements Sources/Phrased/Resources/ Package.swift
