...
人说话或打字，往往表达的不是最清晰的版本——打字太快发现错了消息已经发出去；想向某人表达什么，措辞却要想半天；或者知道自己想说什么，但落到文字里总差一口气：语气不对、结构散乱、太口语化。现有 AI 输入工具（如 Wispr Flow）主打语音转文字，但日常更多的还是打字场景，而文字润色几乎没有产品在做。

Phrased 就是为了填补这个空白：语音或文字进来，AI 理解你的意图，改写成随时可用的表达，直接注入光标位置。完全开源，支持任意本地或云端模型，数据不离开你的设备。
