...
人说话或打字，往往表达的不是最清晰的版本——打字太快发现错了消息已经发出去；想向某人表达什么，措辞却要想半天；或者知道自己想说什么，但落到文字里总差一口气：语气不对、结构散乱、太口语化。现有语音工具（如 Wispr Flow）主打语音转文字，改写工具（如 WritingTools）则需要先选中文字再操作——都是事后补救，而不是在输入时就介入。

Phrased 的思路不同：语音或文字进来，AI 理解意图并改写，结果直接注入光标位置，在你发送之前。完全开源，支持任意本地或云端模型，数据不离开你的设备。
