...
现有语音输入工具，要么订阅费高昂且绑定特定云端服务，要么只能原样转写、无法润色表达，要么不支持中文或本地模型。Phrased 是一个完全开源的替代方案：全局快捷键唤出浮动面板，语音或文字输入，AI 即时改写，文字直接注入任意应用的光标位置。支持任意 OpenAI 兼容的语言模型和语音服务，本地或云端均可，数据不离开你的设备。
